...
export function EditSeriesForm({ series, opponents, players, maps, teamId }: EditSeriesFormProps) {
  ...
  const addGame = () => {
    ...
    const defaultStats = lastGame
      ? pad(lastGame.stats.map((s) => ({ ...emptyStats(), player_id: s.player_id })))
      : pad(players.filter((p) => p.is_default).map((p) => ({ ...emptyStats(), player_id: p.id })));

    const defaultOpponentStats = lastGame
      ? pad(lastGame.opponent_stats.map((s) => ({ ...emptyStats(), player_id: s.player_id })))
      : pad((opponents.find((o) => o.id === opponentId)?.opponent_players ?? []).filter((p) => p.is_default).map((p) => ({ ...emptyStats(), player_id: p.id })));
